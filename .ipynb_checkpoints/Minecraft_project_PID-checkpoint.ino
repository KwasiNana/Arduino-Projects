...
// These values control how the system corrects for error.
// Kp: Proportional gain (for current error).
// Ki: Integral gain (for accumulated error over time).
// Kd: Derivative gain (for anticipating future error).
double Kp = 2.5;
double Ki = 0.1;
double Kd = 0.1;
...
// It sends commands via I2C to configure the sensor and retrieve readings.
void gyro_signals(void) {
  ...
  // This converts the raw data into G-force readings.
  AccX = (float)AccXLSB / 4096;
  AccY = (float)AccYLSB / 4096;
  AccZ = (float)AccZLSB / 4096;

  // Subtract the calibration values to normalize the readings.
  // This ensures the readings are relative to the calibrated "resting" position.
  AccX -= aX;
  AccY -= aY;
  AccZ -= aZ;
}
...
void setup() {
  ...
  // --- MPU6050 Calibration (Project Requirement) ---
  // This loop calibrates the accelerometer to find the average offset
  // for a neutral, non-tilted position. This is essential for
...
void loop() {
  ...
  // --- In-Game Action Control via Pressure Sensor (Project Requirement) ---
  // This block handles mouse clicks for "fighting" and "blocking" based on breath pressure.
  if ((air - flow) > 25 && (air - flow) < 600) {
    factor = 1;
    Mouse.click(MOUSE_LEFT); // Left-click for attacking.
    Serial.println("fight");
  } else if ((air - flow) < -2800) {
    factor = -1;
    Mouse.click(MOUSE_RIGHT); // Right-click for blocking or placing.
    Serial.println("block");
  } else {
    factor = 0;
  }

  // --- WASD Movement via Head Tilt (Project Requirement) ---
  // This block implements WASD control by checking thresholds on accelerometer readings.
  // A PID controller can be implemented here for smoother movement, though the current
  // simple threshold logic works for basic on/off control.

  // 'w' (Forward): Head tilted forward (negative Z acceleration).
  if (AccZ < -0.4) {
    Keyboard.press('w');
  } else {
    Keyboard.release('w');
  ...
  // 's' (Backward): Head tilted backward (positive Z acceleration).
  if (AccZ > 0.1) {
    Keyboard.press('s');
  } else {
    Keyboard.release('s');
  ...
  // 'd' (Strafe Right): Head tilted to the right (negative Y acceleration).
  if (AccY < -0.2) {
    Keyboard.press('d');
  } else {
    Keyboard.release('d');
  ...
  // 'a' (Strafe Left): Head tilted to the left (positive Y acceleration).
  if (AccY > 0.2) {
    Keyboard.press('a');
  } else {
    Keyboard.release('a');
  ...
  // --- Jump and Fly Control via Pressure Sensor (Project Requirement) ---
  // A moderate breath (pressure > 600) triggers a single spacebar press.
  if ((air - flow) > 600) {
    Keyboard.press(' ');
    Serial.println("Jump");
  ...
  // A hard breath (pressure > 2000) sends multiple spacebar presses to trigger flying.
  if ((air - flow) > 2000) {
    Keyboard.press(' ');
    Keyboard.release(' ');
    Keyboard.press(' ');
  ...
  // --- PID Control for Mouse Look (NEW) ---
  // This is a more advanced way to handle the camera, making it smoother than the
  // simple tilt-to-turn method.
  
  // X-axis (Pitch): Looking up and down
  double errorX = AccX; // The "error" is the difference from the neutral position.
  integralX += errorX; // Sum up the errors over time.
  double derivativeX = errorX - previous_errorX; // Calculate the rate of change.
  // Calculate the PID output and scale it for mouse movement.
  int mouse_move_X = (int)(Kp * errorX + Ki * integralX + Kd * derivativeX) * 10;
  
  // Y-axis (Yaw): Looking left and right
  double errorY = AccY;
  integralY += errorY;
  double derivativeY = errorY - previous_errorY;
  int mouse_move_Y = (int)(Kp * errorY + Ki * integralY + Kd * derivativeY) * 10;

  // Use the PID output to move the mouse. The Mouse.move() function
